"""Train DeepSeek OCR from a config-driven command."""
...
def train_deepseek_step(
    dataset_info: dict[str, str],
    config_path: str,
    output_dir: str,
    deepseek_model_revision: str = "",
) -> dict[str, str | float]:
    """Train DeepSeek OCR with the provided configuration."""
    variables = {
        **dataset_info,
    ...
    mlflow.log_param("deepseek_config", config_path)
    for key, value in variables.items():
        mlflow.log_param(f"deepseek_{key}", value)

    logger.info("Starting DeepSeek training with %s", config_path)
    result = run_training_from_config(config_path, variables)

    for key, value in result.items():
        if isinstance(value, float):
            mlflow.log_metric(f"deepseek_{key}", value)
